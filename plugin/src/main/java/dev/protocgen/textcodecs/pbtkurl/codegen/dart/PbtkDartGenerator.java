...
public class PbtkDartGenerator implements LanguageGenerator {
  ...
  private void emitFieldCount(CodeWriter w, ProtoField field) {
    ...
    if (field.isMap()) {
      ...
      w.line("if (!%s.isNaN && !%s.isInfinite) count++;", dartField, dartField);
    } else {
      w.line("count++;");
    }
  }
  ...
  private void emitScalarAppend(CodeWriter w, ProtoField field, String dartField, int fieldNum) {
    ...
    switch (type) {
      ...
      case TYPE_FLOAT:
        w.block(
            "if (!%s.isNaN && !%s.isInfinite)".formatted(dartField, dartField),
    ...
    w.block(
        ...
        () -> {
          ...
          if (field.getMapValueType() == FieldDescriptorProto.Type.TYPE_MESSAGE) {
            ...
            w.block(
                "if (!(entry.value as num).isNaN && !(entry.value as num).isInfinite)",
