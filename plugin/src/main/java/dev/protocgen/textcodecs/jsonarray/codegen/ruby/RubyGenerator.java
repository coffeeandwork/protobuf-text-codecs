...
/**
 * Ruby language code generator. Produces Ruby source files with stdlib json-based JSON array
 * serialization.
 */
public class RubyGenerator implements LanguageGenerator {
  ...
  public List<CodeGeneratorResponse.File> generate(ProtoFile file, TypeRegistry registry) {
    RubyCodeEmitter codeEmitter = new RubyCodeEmitter(typeMapper, nameResolver, registry);
    List<CodeGeneratorResponse.File> result = new ArrayList<>();

    // Generate a Ruby file for each top-level message
    ...
    // Generate a Ruby file for each top-level enum
