...
/**
 * Generates the serialize() method body for TypeScript classes. Identical logic to JavaScript but
 * with typed return values and typed locals.
 */
public class TypeScriptSerializerGenerator {

  private final TypeScriptTypeMapper typeMapper;
  private final JavaScriptNameResolver nameResolver;

  public TypeScriptSerializerGenerator(
  ...
  private void emitMapSerialize(CodeWriter w, ProtoField field, String jsField) {
    ...
    w.block(
        ...
        () -> {
          if (stringKey) {
            w.line("const mapObj: Record<string, any> = {};");
            w.block(
            ...
            w.line("const mapArr: any[][] = [];");
            w.block(
                "for (const [key, value] of Object.entries(" + jsField + "))",
                () -> {
                  w.line("const pair: any[] = [];");
    ...
    String encodeExpr =
        String.format(
            "(typeof Buffer !== 'undefined' ? Buffer.from(%s).toString('base64') "
                + ": (() => { let binary = ''; for (let i = 0; i < %s.length; i++) binary += String.fromCharCode((%s as unknown as number[])[i]); return btoa(binary); })())",
