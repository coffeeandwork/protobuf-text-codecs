...
/**
 * TypeScript language code generator. Produces TypeScript source files with typed JSON array
 * serialization. Shares most logic with JavaScript via the TypeScriptCodeEmitter which extends
 * JavaScriptCodeEmitter.
 */
public class TypeScriptGenerator implements LanguageGenerator {
  ...
  private final TypeScriptCodeEmitter codeEmitter =
      new TypeScriptCodeEmitter(typeMapper, nameResolver);
  ...
  public List<CodeGeneratorResponse.File> generate(ProtoFile file, TypeRegistry registry) {
    ...
    // Generate a .ts file for each top-level message
    ...
    // Generate a .ts file for each top-level enum
