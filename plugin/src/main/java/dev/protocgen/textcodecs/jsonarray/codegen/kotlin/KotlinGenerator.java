...
/**
 * Kotlin language code generator. Produces Kotlin source files with StringBuilder-based JSON array
 * serialization (no external JSON library required).
 */
public class KotlinGenerator implements LanguageGenerator {
  ...
  private final KotlinCodeEmitter codeEmitter = new KotlinCodeEmitter(typeMapper, nameResolver);
  ...
  public List<CodeGeneratorResponse.File> generate(ProtoFile file, TypeRegistry registry) {
    ...
    // Generate a Kotlin enum for each top-level enum
